import * as Router from "koa-router"
import { ParameterizedContext } from "koa"

export const getMockArticles = async (
	ctx: ParameterizedContext<any, Router.IRouterParamContext<any, {}>>
) => {
	ctx.body = [
		{
			id: 20949974,
			title:
				"The beauty of functional languages in deep learning – Clojure and Haskell",
			points: 182,
			user: "wickwavy",
			time: 1568285056,
			time_ago: "4 hours ago",
			comments_count: 63,
			type: "link",
			url:
				"https://www.welcometothejungle.co/fr/articles/btc-deep-learning-clojure-haskell",
			domain: "welcometothejungle.co"
		},
		{
			id: 20951580,
			title: "Two Browsers Walked into a Scrollbar",
			points: 22,
			user: "robin_reala",
			time: 1568297276,
			time_ago: "an hour ago",
			comments_count: 4,
			type: "link",
			url: "https://www.filamentgroup.com/lab/scrollbars/",
			domain: "filamentgroup.com"
		},
		{
			id: 20951573,
			title: "College, Calculus, and the Problem with the SAT",
			points: 23,
			user: "kaboro",
			time: 1568297241,
			time_ago: "an hour ago",
			comments_count: 13,
			type: "link",
			url: "https://www.wired.com/story/college-calculus-problem-with-the-sat/",
			domain: "wired.com"
		},
		{
			id: 20950126,
			title: "Attempto Controlled English",
			points: 109,
			user: "solarkraft",
			time: 1568286577,
			time_ago: "4 hours ago",
			comments_count: 20,
			type: "link",
			url: "https://en.wikipedia.org/wiki/Attempto_Controlled_English",
			domain: "en.wikipedia.org"
		},
		{
			id: 20950865,
			title: "'bic': A C interpreter and API explorer",
			points: 68,
			user: "hexagonal-sun",
			time: 1568292893,
			time_ago: "2 hours ago",
			comments_count: 9,
			type: "link",
			url: "https://github.com/hexagonal-sun/bic",
			domain: "github.com"
		},
		{
			id: 20948826,
			title:
				"Can I Email: Caniuse for email clients. Support tables for HTML and CSS in email",
			points: 367,
			user: "heidijavi",
			time: 1568272441,
			time_ago: "8 hours ago",
			comments_count: 119,
			type: "link",
			url: "https://www.caniemail.com/",
			domain: "caniemail.com"
		},
		{
			id: 20951665,
			title: "Anatomy of a Large-Scale Hypertextual Web Search Engine (1998)",
			points: 15,
			user: "bookofjoe",
			time: 1568297699,
			time_ago: "an hour ago",
			comments_count: null,
			type: "link",
			url: "http://infolab.stanford.edu/~backrub/google.html",
			domain: "infolab.stanford.edu"
		},
		{
			id: 20951444,
			title: "Ask HN: R U OK?",
			points: 149,
			user: "quantumwoke",
			time: 1568296446,
			time_ago: "an hour ago",
			comments_count: undefined,
			type: "ask",
			url: "item?id=20951444"
		},
		{
			id: 20950535,
			title: "Energy of the 229 Th nuclear clock transition",
			points: null,
			user: "Tomte",
			time: 1568290455,
			time_ago: "3 hours ago",
			comments_count: 7,
			type: "link",
			url: "https://www.nature.com/articles/s41586-019-1533-4",
			domain: "nature.com"
		},
		{
			id: 20949956,
			title:
				"Crispr Gene-Editing May Offer Path to Cure for HIV, First Published Report Shows",
			points: undefined,
			user: "pseudolus",
			time: 1568284831,
			time_ago: "4 hours ago",
			comments_count: 30,
			type: "link",
			url:
				"https://www.npr.org/sections/health-shots/2019/09/11/759369190/crispr-gene-editing-may-offer-path-to-cure-for-hiv-first-published-report-shows",
			domain: "npr.org"
		},
		{
			id: 20950034,
			title: "Is Another Interstellar Visitor Headed Our Way?",
			points: 62,
			user: "dchest",
			time: 1568285665,
			time_ago: "4 hours ago",
			comments_count: 35,
			type: "link",
			url:
				"https://www.skyandtelescope.com/astronomy-news/possible-interstellar-comet-headed-our-way/",
			domain: "skyandtelescope.com"
		},
		{
			id: 20951619,
			title:
				"Anti-vax movement effectively reversing decades of progress internationally",
			points: 30,
			user: "rahuldottech",
			time: 1568297516,
			time_ago: "an hour ago",
			comments_count: 10,
			type: "link",
			url:
				"https://qz.com/1706261/the-anti-vax-movement-is-reversing-decades-of-scientific-progress/",
			domain: "qz.com"
		},
		{
			id: 20950468,
			title: "Summer heat killed nearly 1,500 in France, officials say",
			points: 78,
			user: "elorant",
			time: 1568289901,
			time_ago: "3 hours ago",
			comments_count: 43,
			type: "link",
			url: "https://www.bbc.com/news/world-europe-49628275",
			domain: "bbc.com"
		},
		{
			id: 20950712,
			title:
				"Israel accused of planting mysterious spy devices near the White House",
			points: 118,
			user: "spzx",
			time: 1568291879,
			time_ago: "2 hours ago",
			comments_count: 70,
			type: "link",
			url:
				"https://www.politico.com/story/2019/09/12/israel-white-house-spying-devices-1491351",
			domain: "politico.com"
		},
		{
			id: 20948613,
			title: "Parasite SEO",
			points: 111,
			user: "JohnnyPageviews",
			time: 1568269787,
			time_ago: "9 hours ago",
			comments_count: 34,
			type: "link",
			url:
				"https://affiliateinsights.com/subdomain-leasing-affiliate-white-label/",
			domain: "affiliateinsights.com"
		},
		{
			id: 20935949,
			title: "Fast radio bursts from space have baffled scientists for years",
			points: 4,
			user: "prostoalex",
			time: 1568168066,
			time_ago: "2 days ago",
			comments_count: 0,
			type: "link",
			url:
				"https://www.nbcnews.com/mach/science/fast-radio-bursts-space-have-baffled-scientists-years-explanation-may-ncna1049216",
			domain: "nbcnews.com"
		},
		{
			id: 20947631,
			title: "Rock Pi X Intel Cherry Trail Board",
			points: 134,
			user: "watchdogtimer",
			time: 1568258777,
			time_ago: "12 hours ago",
			comments_count: 81,
			type: "link",
			url:
				"https://www.cnx-software.com/2019/09/11/rock-pi-x-low-cost-intel-sbc/",
			domain: "cnx-software.com"
		},
		{
			id: 20924057,
			title:
				"Seeing What Lives in Rainwater by Making a Closed Ecosystem [video]",
			points: 7,
			user: "RodgerTheGreat",
			time: 1568075752,
			time_ago: "3 days ago",
			comments_count: 2,
			type: "link",
			url: "https://www.youtube.com/watch?v=n7iyPp3r9HI",
			domain: "youtube.com"
		},
		{
			id: 20921090,
			title: "A Low-Fare Loophole for International Travelers",
			points: 45,
			user: "prostoalex",
			time: 1568055112,
			time_ago: "3 days ago",
			comments_count: 32,
			type: "link",
			url:
				"https://www.wsj.com/articles/a-low-fare-loophole-for-international-travelers-1540992568?mod=rsswn",
			domain: "wsj.com"
		},
		{
			id: 20941473,
			title:
				"Two California hikers found a family's message in a bottle and helped save them",
			points: 69,
			user: "curtis",
			time: 1568217125,
			time_ago: "a day ago",
			comments_count: 34,
			type: "link",
			url:
				"https://www.cnn.com/2019/09/11/us/california-hikers-bottle-message-search-trnd/index.html",
			domain: "cnn.com"
		},
		{
			id: 20945856,
			title: "California Approves Statewide Rent Control",
			points: 561,
			user: "dawhizkid",
			time: 1568244294,
			time_ago: "16 hours ago",
			comments_count: 806,
			type: "link",
			url:
				"https://www.nytimes.com/2019/09/11/business/economy/california-rent-control.html",
			domain: "nytimes.com"
		},
		{
			id: 20951263,
			title: "The Drawbacks of Python",
			points: 32,
			user: "iflywithbook",
			time: 1568295439,
			time_ago: "an hour ago",
			comments_count: 33,
			type: "link",
			url:
				"https://www.quora.com/Python-programming-language-What-are-some-of-the-drawbacks-of-Python/answer/Dave-Wade-Stein?ch=10&share=9ec1027c&srid=43O3",
			domain: "quora.com"
		},
		{
			id: 20933867,
			title: "Chinese Pillow History",
			points: 19,
			user: "benbreen",
			time: 1568150350,
			time_ago: "2 days ago",
			comments_count: 7,
			type: "link",
			url: "https://wellcomecollection.org/articles/XV_E7BEAACIAo9Vz",
			domain: "wellcomecollection.org"
		},
		{
			id: 20948434,
			title: "Ban children from gambling in games, MPs say",
			points: 162,
			user: "MarcScott",
			time: 1568267652,
			time_ago: "9 hours ago",
			comments_count: 183,
			type: "link",
			url: "https://www.bbc.co.uk/news/technology-49661870",
			domain: "bbc.co.uk"
		},
		{
			id: 20944005,
			title: "Water found on a potentially life-friendly alien planet",
			points: 552,
			user: "Leace",
			time: 1568231669,
			time_ago: "19 hours ago",
			comments_count: 345,
			type: "link",
			url:
				"https://www.nationalgeographic.com/science/2019/09/first-water-found-in-habitable-exoplanets-atmosphere-hubble-kepler-k2-18b/",
			domain: "nationalgeographic.com"
		},
		{
			id: 20947222,
			title:
				"Immer is a library of persistent and immutable data structures written in C++",
			points: 77,
			user: "fortran77",
			time: 1568255312,
			time_ago: "13 hours ago",
			comments_count: 41,
			type: "link",
			url: "https://github.com/arximboldi/immer",
			domain: "github.com"
		},
		{
			id: 20949763,
			title: "Scientists Find the Skull of Humanity’s Ancestor, on a Computer",
			points: 11,
			user: "rbanffy",
			time: 1568282443,
			time_ago: "5 hours ago",
			comments_count: 13,
			type: "link",
			url:
				"https://www.nytimes.com/2019/09/10/science/human-ancestor-skull-computer.html",
			domain: "nytimes.com"
		},
		{
			id: 20945819,
			title: "A high-speed network driver written in C, Rust, Go, C#, Java",
			points: 386,
			user: "Sindisil",
			time: 1568243819,
			time_ago: "16 hours ago",
			comments_count: 117,
			type: "link",
			url: "https://github.com/ixy-languages/ixy-languages",
			domain: "github.com"
		},
		{
			id: 20922887,
			title: "Using ML for Automated RF Side-Channel Attack Classification",
			points: 19,
			user: "ldayley",
			time: 1568065968,
			time_ago: "3 days ago",
			comments_count: 2,
			type: "link",
			url: "https://leveldown.de/blog/tensorflow-sidechannel-analysis/",
			domain: "leveldown.de"
		},
		{
			id: 20944148,
			title: "What is the new Apple U1 chip?",
			points: 476,
			user: "MaysonL",
			time: 1568232772,
			time_ago: "19 hours ago",
			comments_count: 130,
			type: "link",
			url:
				"https://www.quora.com/What-is-the-new-Apple-U1-chip-and-why-is-it-important/answer/Brian-Roemmele?ch=10&share=7e467f08&srid=Pi3",
			domain: "quora.com"
		}
	]
}
